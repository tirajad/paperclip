...
export function PluginSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: plugin?.manifestJson?.displayName ?? plugin?.packageName ?? "รายละเอียดปลั๊กอิน" },
    ]);
  ...
  if (pluginLoading) {
    return <div className="p-4 text-sm text-muted-foreground">กำลังโหลดรายละเอียดปลั๊กอิน...</div>;
  }
  ...
  const statusVariant =
    plugin.status === "ready"
      ...
      : plugin.status === "error"
        ? "destructive"
        : "secondary";
  const pluginDescription = plugin.manifestJson.description || "ไม่มีคำอธิบาย";
        ...
        <TabsContent value="configuration" className="space-y-6">
          <div className="space-y-8">
            <section className="space-y-5">
              <h2 className="text-base font-semibold">เกี่ยวกับ</h2>
              <div className="grid gap-8 lg:grid-cols-[minmax(0,1.4fr)_minmax(220px,0.8fr)]">
                <div className="space-y-2">
                  <h3 className="text-sm font-medium text-muted-foreground">คำอธิบาย</h3>
                  <p className="text-sm leading-6 text-foreground/90">{pluginDescription}</p>
                </div>
                <div className="space-y-4 text-sm">
                  <div className="space-y-1.5">
                    <h3 className="font-medium text-muted-foreground">ผู้พัฒนา</h3>
                    <p className="text-foreground">{plugin.manifestJson.author}</p>
                  </div>
                  <div className="space-y-2">
                    <h3 className="font-medium text-muted-foreground">หมวดหมู่</h3>
                    <div className="flex flex-wrap gap-2">
                      {plugin.categories.length > 0 ? (
                        ...
                        <span className="text-foreground">ไม่มี</span>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <section className="space-y-4">
              <div className="space-y-1">
                <h2 className="text-base font-semibold">การตั้งค่า</h2>
              </div>
              {hasCustomSettingsPage ? (
                ...
                <p className="text-sm text-muted-foreground">
                  ปลั๊กอินนี้ไม่ต้องการการตั้งค่าใด ๆ
                </p>
              )}
            </section>
          </div>
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_320px]">
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    กระบวนการเวิร์กเกอร์ งานที่ตั้งเวลาไว้ และการส่ง Webhook
                  </CardDescription>
                </CardHeader>
                    ...
                    <>
                      <div>
                        ...
                        {dashboardData.worker ? (
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            ...
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">RPC ที่รอดำเนินการ</span>
                            ...
                            {dashboardData.worker.totalCrashes > 0 && (
                              <>
                                ...
                                {dashboardData.worker.lastCrashAt && (
                                  <div className="flex justify-between col-span-2">
                                    <span className="text-muted-foreground">แครชล่าสุด</span>
                          ...
                          <p className="text-sm text-muted-foreground italic">ไม่มีกระบวนการเวิร์กเกอร์ที่ลงทะเบียนไว้</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {dashboardData.recentJobRuns.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">ยังไม่มีบันทึกการรันงาน</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {dashboardData.recentWebhookDeliveries.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">ยังไม่มีบันทึกการส่ง Webhook</p>
                        )}
                      </div>

                      <div className="flex items-center gap-1.5 border-t border-border/50 pt-2 text-xs text-muted-foreground">
                        <Clock className="h-3 w-3" />
                        ตรวจสอบล่าสุด: {new Date(dashboardData.checkedAt).toLocaleTimeString()}
                      </div>
                    </>
                  ) : (
                    <p className="text-sm text-muted-foreground">
                      การวินิจฉัยรันไทม์ไม่พร้อมใช้งานในขณะนี้
                    </p>
                  )}
                </CardContent>
              </Card>
              ...
              {recentLogs && recentLogs.length > 0 ? (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>{recentLogs.length} รายการบันทึกล่าสุด</CardDescription>
                  </CardHeader>
                  <CardContent>
            ...
            <div className="space-y-6">
              <Card>
                ...
                <CardContent>
                  {healthLoading ? (
                    <p className="text-sm text-muted-foreground">กำลังตรวจสอบสุขภาพ...</p>
                  ) : healthData ? (
                    <div className="space-y-4 text-sm">
                      <div className="flex items-center justify-between">
                        <span className="text-muted-foreground">โดยรวม</span>
                    ...
                    <div className="space-y-3 text-sm text-muted-foreground">
                      ...
                      <p>การตรวจสอบสุขภาพจะทำงานเมื่อปลั๊กอินพร้อมใช้งาน</p>
                      {plugin.lastError ? (
                        <div className="break-words rounded border border-destructive/20 bg-destructive/10 p-2 text-xs text-destructive">
                          {plugin.lastError}
                        </div>
              ...
              <Card>
                ...
                <CardContent>
                  {pluginCapabilities.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground italic">ไม่มีการร้องขอสิทธิ์พิเศษ</p>
                  )}
                </CardContent>
              </Card>
            </div>
...
function PluginConfigForm({ pluginId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      setSaveMessage({ type: "success", text: "บันทึกการตั้งค่าแล้ว" });
      setTestResult(null);
    ...
    onError: (err: Error) => {
      setSaveMessage({ type: "error", text: err.message || "บันทึกการตั้งค่าล้มเหลว" });
    },
  });
  ...
  const testMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.valid) {
        setTestResult({ type: "success", text: "การทดสอบการตั้งค่าผ่านแล้ว" });
      } else {
        setTestResult({ type: "error", text: result.message || "การทดสอบการตั้งค่าล้มเหลว" });
      }
    },
    onError: (err: Error) => {
      setTestResult({ type: "error", text: err.message || "การทดสอบการตั้งค่าล้มเหลว" });
    },
  });
