...
export function Approvals() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: "การอนุมัติ" }]);
  }, [setBreadcrumbs]);
  ...
  if (!selectedCompanyId) {
    return <p className="text-sm text-muted-foreground">กรุณาเลือกบริษัทก่อน</p>;
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <Tabs value={statusFilter} onValueChange={(v) => navigate(`/approvals/${v}`)}>
          <PageTabBar items={[
            { value: "pending", label: <>รอดำเนินการ{pendingCount > 0 && (
              <span className={cn(
      ...
      {filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            {statusFilter === "pending" ? "ไม่มีการอนุมัติที่รอดำเนินการ" : "ยังไม่มีการอนุมัติ"}
          </p>
        </div>
      )}
