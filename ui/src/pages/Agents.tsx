...
export function Agents() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: "เอเจนต์" }]);
  }, [setBreadcrumbs]);

  if (!selectedCompanyId) {
    return <EmptyState icon={Bot} message="เลือกบริษัทเพื่อดูเอเจนต์" />;
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {filtered.length > 0 && (
        <p className="text-xs text-muted-foreground">{filtered.length} เอเจนต์</p>
      )}
      ...
      {agents && agents.length === 0 && (
        <EmptyState
          icon={Bot}
          message="สร้างเอเจนต์แรกของคุณเพื่อเริ่มต้นใช้งาน"
          action="เอเจนต์ใหม่"
      ...
      {effectiveView === "list" && agents && agents.length > 0 && filtered.length === 0 && (
        <p className="text-sm text-muted-foreground text-center py-8">
          ไม่มีเอเจนต์ที่ตรงกับตัวกรองที่เลือก
        </p>
      )}
  ...
  return (
    ...
    >
      ...
      <span className="text-[11px] font-medium text-blue-600 dark:text-blue-400">
        ถ่ายทอดสด{liveCount > 1 ? ` (${liveCount})` : ""}
      </span>
    </Link>
  );
}
