...
export function MobileBottomNav({ visible }: MobileBottomNavProps) {
  ...
  const items = useMemo<MobileNavItem[]>(
    () => [
      { type: "link", to: "/dashboard", label: "หน้าหลัก", icon: House },
      { type: "link", to: "/issues", label: "งาน", icon: CircleDot },
      { type: "action", label: "สร้าง", icon: SquarePen, onClick: () => openNewIssue() },
      { type: "link", to: "/agents/all", label: "เอเจนต์", icon: Users },
      {
        type: "link",
        to: "/inbox",
        label: "กล่องข้อความ",
