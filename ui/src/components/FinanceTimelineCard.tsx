...
export function FinanceTimelineCard({
  rows,
  emptyMessage = "ไม่มีเหตุการณ์ทางการเงินในช่วงเวลานี้",
}: FinanceTimelineCardProps) {
  return (
    <Card>
      <CardHeader className="px-4 pt-4 pb-1">
        <CardTitle className="text-base">เหตุการณ์ทางการเงินล่าสุด</CardTitle>
        <CardDescription>การเติมเงิน, ค่าธรรมเนียม, เครดิต, ข้อผูกพัน และค่าใช้จ่ายอื่นที่ไม่ใช่คำขอ</CardDescription>
      </CardHeader>
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 space-y-2">
                  ...
                  {(row.description || row.externalInvoiceId || row.region || row.pricingTier) && (
                    <div className="space-y-1 text-xs text-muted-foreground">
                      ...
                      {row.externalInvoiceId ? <div>ใบแจ้งหนี้ {row.externalInvoiceId}</div> : null}
                      {row.region ? <div>ภูมิภาค {row.region}</div> : null}
                      {row.pricingTier ? <div>ระดับราคา {row.pricingTier}</div> : null}
                    </div>
                  )}
                </div>
                <div className="text-right tabular-nums">
                  ...
                  {row.estimated ? <div className="text-[11px] uppercase tracking-[0.12em] text-amber-600">ประมาณการ</div> : null}
                </div>
              </div>
            </div>
          ))
