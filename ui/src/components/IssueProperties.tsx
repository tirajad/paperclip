...
export function IssueProperties({ issue, onUpdate, inline }: IssuePropertiesProps) {
  ...
  const labelsTrigger = (issue.labels ?? []).length > 0 ? (
    ...
    <>
      ...
      <span className="text-sm text-muted-foreground">ไม่มีป้ายกำกับ</span>
    </>
  );
    ...
    <>
      ...
      <div className="mt-2 border-t border-border pt-2 space-y-1">
        ...
        >
          ...
          {createLabel.isPending ? "กำลังสร้าง…" : "สร้างป้ายกำกับ"}
        </button>
      </div>
    </>
  );
  ...
  const assigneeTrigger = assignee ? (
    ...
    <>
      <User className="h-3.5 w-3.5 text-muted-foreground" />
      <span className="text-sm text-muted-foreground">ยังไม่ได้มอบหมาย</span>
    </>
  );
  ...
  const assigneeContent = (
    <>
      <input
        className="w-full px-2 py-1.5 text-xs bg-transparent outline-none border-b border-border mb-1 placeholder:text-muted-foreground/50"
        placeholder="ค้นหาผู้รับมอบหมาย..."
      ...
      <div className="max-h-48 overflow-y-auto overscroll-contain">
        ...
        {issue.createdByUserId && issue.createdByUserId !== currentUserId && (
          ...
          >
            <User className="h-3 w-3 shrink-0 text-muted-foreground" />
            {creatorUserLabel ? `มอบหมายให้ ${creatorUserLabel}` : "มอบหมายให้ผู้ร้องขอ"}
          </button>
        )}
        {sortedAgents
          .filter((a) => {
  ...
  const projectTrigger = issue.projectId ? (
    ...
    <>
      ...
      <span className="text-sm text-muted-foreground">ไม่มีโปรเจกต์</span>
    </>
  );
        ...
        {(issue.createdByAgentId || issue.createdByUserId) && (
          <PropertyRow label="สร้างโดย">
            {issue.createdByAgentId ? (
              ...
              <>
                <User className="h-3.5 w-3.5 text-muted-foreground" />
                <span className="text-sm">{creatorUserLabel ?? "ผู้ใช้"}</span>
              </>
            )}
          </PropertyRow>
        )}
        {issue.startedAt && (
          <PropertyRow label="เริ่มเมื่อ">
            <span className="text-sm">{formatDate(issue.startedAt)}</span>
          </PropertyRow>
        )}
        {issue.completedAt && (
          <PropertyRow label="เสร็จเมื่อ">
